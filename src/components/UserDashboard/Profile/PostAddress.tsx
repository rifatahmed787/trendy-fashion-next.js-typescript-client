...
const PostAddress = () => {
  ...
  useEffect(() => {
    if (addressIError && AddressError && "data" in AddressError) {
      ...
      setAlertMessages(addressdata?.message ?? "Address added successfully")
      if (addressIsSuccess) {
        setTimeout(() => {
          onClose();
        }, 3000);
  ...
  }, [AddressError, addressIError, addressIsSuccess, addressdata?.message, onClose]);
  return (
    <Modal isOpen={isOpen("postAddree")} onClose={onClose}>
      <ModalBody>
        <ModalHeader title="Address" onClose={onClose} />
